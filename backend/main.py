...
async def run(request: Request) -> JSONResponse:
    ...
    user_msg = user_payload(job_id, prompt.strip(), project)
    combined = sys_msg + "\n" + user_msg

    if _cap.would_exceed(combined):
        return fail("cap", "Session spend cap reached. No more runs until the proxy restarts.")

    if _mock_on():
        print("studio run cost_usd=0.000000 spent_usd=%.6f mock=1" % _cap.spent_usd, flush=True)
        return JSONResponse(mock_payload(job_id, project))
    ...
    if not api_key:
        return fail("upstream", "The model did not respond. Try Run again.")
    ...
    try:
        ...
        return fail("timeout", "That run timed out. Try Run again.")
    ...
    print(
        "studio run cost_usd=%.6f spent_usd=%.6f" % (cost, _cap.spent_usd),
        flush=True,
    )

    parsed = shaped_json(job_id, extract_json(text))
    return JSONResponse({"ok": True, "text": text, "json": parsed})
